...
var ProcessDiagramGenerator = {	
	...
	showFlowInfo: function(flow){
		...
		var TPL_FLOW_INFO = '<div>{flow}</div>' 
				+ '<div><b>来源节点Id</b>: {sourceActivityId}</div>'
				+ '<div><b>目标节点Id</b>: {destinationActivityId}</div>'
				+ '<div><b>是否默认</b>: {isDefault}</div>'
				+ '<div><b>条件</b>: {isConditional}</div>'
	...
	showActivityInfo: function(activity,tasks){
		var diagramInfo = $("#" + this.options.diagramInfoId);
		if (!diagramInfo) return;
		var taskType=eval("tasks."+activity.getProperty("type"));
		if(!taskType){
			taskType=activity.getProperty("type");
		}
		var values = {
