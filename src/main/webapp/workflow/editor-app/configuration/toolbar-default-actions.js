    ...
    ACTIONS: {
        ...
        saveModel: function (services) {
            ...
            var params = {
                json_xml: json,
                svg_xml: svgDOM,
                name: services.$scope.modelData.name,
                description: services.$scope.modelData.description
            };
            ...
            services.$http({    method: 'PUT',
                ...
                url: KISBPM.URL.putModel(services.$scope.editor.getModelMetaData().modelId)})
                .success(function (data, status, headers, config) {
                    ...
                    var saveEvent = {
                        ...
                        model: params,
                        modelId: services.$scope.editor.getModelMetaData().modelId,
        	...
        	/**
			 * 修改不弹出模型保存窗口 var modal = services.$modal({ backdrop: true,
			 * keyboard: true, template:
			 * 'editor-app/popups/save-model.html?version=' + Date.now(), scope:
			 * services.$scope });
			 */
        },
