...
UE.plugins['text'] = function () {
	...
	me.addListener( 'mouseover', function( t, evt ) {
        ...
        var qPlugins = el.getAttribute('qPlugins');
		if ( /input/ig.test( el.tagName ) && qPlugins==thePlugins) {
...
UE.plugins['radios'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        var el = evt.target || evt.srcElement;
        var qPlugins = el.getAttribute('qPlugins');
        if ( /span/ig.test( el.tagName ) && qPlugins==thePlugins) {
            var html = popup.formatHtml(
...
UE.plugins['select'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        var el = evt.target || evt.srcElement;
        var qPlugins = el.getAttribute('qPlugins');
        if ( /select|span/ig.test( el.tagName ) && qPlugins==thePlugins) {
            var html = popup.formatHtml(
...
UE.plugins['progressbar'] = function () {
    ...
    me.addListener( 'mouseover', function( t, evt ) {
        ...
        var el = evt.target || evt.srcElement;
        var qPlugins = el.getAttribute('qPlugins');
        if ( /img/ig.test( el.tagName ) && qPlugins==thePlugins) {
            var html = popup.formatHtml(
...
UE.plugins['qf'] = function () {
    var me = this,thePlugins = 'qf';
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl:this.options.UEDITOR_HOME_URL + UE.formDesignUrl+'/index.html',
                name:thePlugins,
                editor:this,
                title: '表单设计器 - 清单',
...
UE.plugins['qf_template'] = function () {
    var me = this,thePlugins = 'qf_template';
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
...
UE.registerUI('button_qf',function(editor,uiName){
    if(!this.options.qftool)
    {
        return false;
    }
