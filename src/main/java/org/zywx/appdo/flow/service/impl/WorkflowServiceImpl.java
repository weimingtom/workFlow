...
public class WorkflowServiceImpl implements WorkflowService {
	...
	public DataGrid getMyInvolvedProcList(Map<String, Object> paraMap, DataGrid dataGrid) {
		List<HistoricProcessInstance> hpis = historyService.createHistoricProcessInstanceQuery().involvedUser(paraMap.get("userId").toString())
				.orderByProcessInstanceStartTime().desc().listPage((dataGrid.getPage() - 1) * dataGrid.getPageSize(),
	...
	public List<String> queryUsers(String propertity, DelegateExecution execution) {
		...
		if (vars.length != 0) {
			if (vars[0].equals(PropertyTools.getPropertyByKey("superior"))) {
				String userId = "";
				//查询历史中最近操作用户
				List<HistoricTaskInstance> list = historyService.createHistoricTaskInstanceQuery().processInstanceId(execution.getId()).orderByTaskId().desc().list();
				if (list != null) {
					if(list.size()>0){
